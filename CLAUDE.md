...
| `data.rs`   | `DataTable` + `RowBackend` enum (InMemory/Csv/Parquet). Format loaders, Arrow value→string conversion (including all temporal types), Excel date serial conversion |
...
| `ui.rs`     | All rendering at `Buffer` level. `render_table`, `render_status_bar`, `render_tab_bar`, `render_help_screen`. Column layout via `compute_layout`                   |
...
- `show_help: bool` — when true, a modal help screen is rendered. Only `Esc`/`?` are processed; all other keys are ignored (including `q` — it won't quit while help is open).
...
### Arrow value → string conversion (`arrow_value_to_str`)

Located in `data.rs`. Handles all Arrow types found in Parquet files:

| Category  | Types                                            | Output format                  |
| --------- | ------------------------------------------------ | ------------------------------ |
| Numeric   | Int8–64, UInt8–64, Float16–64                    | Standard `to_string()`         |
| String    | StringArray, LargeStringArray                    | Direct `.to_string()`          |
| Boolean   | BooleanArray                                     | `"true"` / `"false"`           |
| Date      | Date32Array, Date64Array                         | `YYYY-MM-DD`                   |
| Timestamp | TimestampSecond/Millisecond/Micro/Nanosecond     | `YYYY-MM-DD HH:MM:SS[.subsec]` |
| Time      | Time32Second/Millisecond, Time64Micro/Nanosecond | `HH:MM:SS[.subsec]`            |
| Duration  | DurationSecond/Millisecond/Micro/Nanosecond      | Fractional seconds (`Ns`)      |

Null values return `""`. The date conversion uses Howard Hinnant's `civil_from_days` algorithm — O(1), correct for the full `i64` range. All division uses `.div_euclid()` / `.rem_euclid()` for correct floor behavior with pre-epoch dates.

Excel dates go through `cell_to_string` → `excel_serial_to_date`, which handles the 1900 leap-year bug (serial 60 = "1900-02-29", serials > 60 decrement by 1).
...
- **CJK handling**: The only way to render text is via `put_text()` in `ui.rs`. It uses `UnicodeWidthChar::width()` to advance cursor position (1 for ASCII, 2 for CJK). Never use `buf[(x, y)].set_char(ch)` directly for multi-char strings — it won't account for CJK width.
- **Width computation**: `UnicodeWidthStr::width()` is used for column widths. CJK characters are width 2.
...
- **Event loop**: `prefetch_range(scroll_row, visible_rows)` is called before each `terminal.draw()`. This ensures the cache covers the current viewport before rendering. For InMemory this is a no-op. Polling uses 50ms timeout: `event::poll(Duration::from_millis(50))`.

### Rendering pipeline (per frame)

1. `render()` in `ui.rs` calculates table area, tab bar height, status bar height
2. `viewport.recalc_dimensions()` — re-fits visible rows/cols to terminal size
3. Entire table area background is filled with `palette.bg`
4. `render_table()` calls `compute_layout()` to get column X positions, then renders header + data rows row-by-row
5. `render_tab_bar()` — only shown when `sheets.len() > 1`; draws `[ Sheet1 | Sheet2 ]`
6. `render_status_bar()` — shows cursor position + status message at bottom
